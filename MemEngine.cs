...
public abstract class MemEngine
{
    ...
    public class MemoryApplication(long handle, ProcessModuleCollection modules)
    {
        ...
        public List<long> SignatureScanning(string moduleName, int[] bytes)
        {
            ...
            {
                ProcessChunk(address + i * 4096, bytes, result);
            }
            return result;
        }
        ...
        private void ProcessChunk(long addr, IReadOnlyList<byte> bytes, ICollection<long> result)
        {
            ...
            {
                var byteRec = buffer[i];
                if (byteRec != bytes[0]) continue;

                var notMatch = false;
                for (var inner = 0; inner < bytes.Count; inner++)
                {
                    if (i + inner < buffer.Length && buffer[i + inner] == bytes[inner]) continue;
                    notMatch = true;
                    break;
                }
        ...
        private void ProcessChunk(long addr, IReadOnlyList<int> bytes, ICollection<long> result)
        {
            var buffer = new byte[4096];
            var bytesRead = 0;
            ReadProcessMemory(handle, addr, buffer, buffer.Length, ref bytesRead);
    ...
    public static MemoryApplication OpenProcess(string processName)
    {
        ...
        var handle = OpenProcess(0x000F0000 | 0x00100000 | 0xFFFF, true, proc.Id);
